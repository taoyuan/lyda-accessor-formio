...
var debug = require('debug')('lyda:accessor:formio');
...
function typeMapper(componentType) {
  let result = null;
  const ct = new RegExp(componentType, 'i');
  _.forEach(TYPE_MAPS, (possibles, type) => {
    if (_.find(possibles, possible => ct.test(possible))) {
      result = type;
      return false;
    }
  });
